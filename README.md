...
**http://3.39.39.22:3000/**

> API 키 없이 **가상계좌(데모 모드)** 로 바로 체험할 수 있습니다.
> 회원가입 후 데모 모드를 활성화하면 실제 시세 기반으로 매매 연습이 가능합니다.
...
### Demo Mode
- API 키 없이 서버의 공유 API 키로 실시간 시세 조회
- 가상 자금으로 매매 연습 (최대 10억원)
- 실제 시세 기반, 실제 자산 미노출
- 악용 방지: 거래 쿨다운, 일일 한도, 계정 수 제한
...
│   ├── demo_guard.py       # Demo mode abuse prevention
...
| `DEMO_UPBIT_ACCESS_KEY` | Demo mode shared Upbit Access Key | No (enables demo API) |
| `DEMO_UPBIT_SECRET_KEY` | Demo mode shared Upbit Secret Key | No (enables demo API) |
| `DEMO_MAX_ACCOUNTS` | Maximum demo accounts allowed | No (default: `50`) |
| `DEMO_TRADE_COOLDOWN_SEC` | Seconds between demo trades | No (default: `5`) |
| `DEMO_MAX_BALANCE` | Maximum demo balance (KRW) | No (default: `1,000,000,000`) |
| `DEMO_DAILY_TRADE_LIMIT` | Max demo trades per day | No (default: `100`) |

## Demo Mode Abuse Prevention

데모 모드는 서버 소유자의 API 키를 공유하여 실시간 시세를 제공합니다.
악용 방지를 위해 다음 제한이 적용됩니다:

| Protection | Description |
|------------|-------------|
| Trade Cooldown | 거래 간 최소 대기 시간 (기본: 5초) |
| Daily Trade Limit | 24시간 내 최대 거래 횟수 (기본: 100회) |
| Balance Cap | 가상 잔고 상한 (기본: 10억원) |
| Account Limit | 동시 데모 계정 수 제한 (기본: 50개) |
| No Real Orders | 데모 모드에서 실제 주문 절대 불가 |
| No Bot Access | 데모 유저는 자동매매 봇 실행 불가 |
| Rate Limiting | IP 기반 분당 120회 요청 제한 |
